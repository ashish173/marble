...
void GLTextureMapper::setTileZoomLevel( int level )
{
    d->m_tileZoomLevel = level;
}

int GLTextureMapper::tileZoomLevel() const
{
    return d->m_tileZoomLevel;
}

void GLTextureMapper::mapTexture( QGLContext *glContext, const ViewportParams *viewport )
{
    ...
    loadVisibleTiles( glContext, viewport );
...
void GLTextureMapper::loadVisibleTiles( QGLContext *glContext, const ViewportParams *viewport )
{
    ...
    const int numXTiles = d->m_tileLoader->tileColumnCount( d->m_tileZoomLevel );
    const int numYTiles = d->m_tileLoader->tileRowCount( d->m_tileZoomLevel );
    ...
    for (int i = startXTile; i < endXTile; ++i) {
        for (int j = startYTile; j < endYTile; ++j) {
            const TileId id( 0, d->m_tileZoomLevel, i % numXTiles, j % numYTiles );
